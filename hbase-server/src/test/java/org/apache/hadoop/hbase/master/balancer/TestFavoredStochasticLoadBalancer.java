...
import static org.apache.hadoop.hbase.favored.FavoredNodesPromoter.FAVORED_ALWAYS_ASSIGN_REGIONS;
...
public class TestFavoredStochasticLoadBalancer extends BalancerTestBase {
  ...
  public static void setupBeforeClass() throws Exception {
    ...
    LOG.debug("ZK assignment mode: " + conf.getBoolean("hbase.assignment.usezk", false));
    conf.setBoolean(FAVORED_ALWAYS_ASSIGN_REGIONS, false);
  }
  ...
  public void test2FavoredNodesDead() throws Exception {
    ...
    TEST_UTIL.waitFor(10000, new Waiter.Predicate<Exception>() {
      @Override
      public boolean evaluate() throws Exception {
        return regionStates.isRegionOnline(region);
      }
    });
  ...
  public void testAllFavoredNodesDead() throws Exception {
    ...
    LOG.debug("FN for candidate region: " + fnm.getFavoredNodes(region));

    final RegionStates regionStates = master.getAssignmentManager().getRegionStates();
    for (Map.Entry<RegionState.State, List<HRegionInfo>> entry : regionStates.getRegionByStateOfTable(tableName).entrySet()) {
      LOG.debug("State: " + entry.getKey() + " Regions: " + entry.getValue());
    }
    TEST_UTIL.waitFor(10000, new Waiter.Predicate<Exception>() {
      @Override
      public boolean evaluate() throws Exception {
        return regionStates.isRegionInState(region, RegionState.State.FAILED_OPEN);
      }
    });

    assertTrue("Region: " + region + " should be RIT",
        regionStates.isRegionInState(region, RegionState.State.FAILED_OPEN));
  ...
  public void testAllFavoredNodesDeadMasterRestarted() throws Exception {
    ...
    TEST_UTIL.waitFor(10000, new Waiter.Predicate<Exception>() {
      @Override
      public boolean evaluate() throws Exception {
        return regionStatesBeforeMaster.isRegionInState(region, RegionState.State.FAILED_OPEN);
      }
    });

    assertTrue("Region: " + region + " should be RIT",
        regionStatesBeforeMaster.isRegionInState(region, RegionState.State.FAILED_OPEN));
  ...
  private void stopServersAndWaitUntilProcessed(List<ServerName> currentFN) throws Exception {
    ...
    for(JVMClusterUtil.RegionServerThread t : cluster.getLiveRegionServerThreads()) {
      LOG.debug("Server: " + t.getRegionServer().getServerName());
    }
